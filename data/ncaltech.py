from torch.utils.data import Dataset
from pathlib import Path
import numpy as np
from torchvision.transforms import Compose, Resize, CenterCrop, ToTensor, Normalize, ToPILImage
from PIL import Image
from data.utils import _convert_image_to_rgb, split_event, make_event_histogram, split_event, center_event
try:
    from torchvision.transforms import InterpolationMode
    BICUBIC = InterpolationMode.BICUBIC
except ImportError:
    BICUBIC = Image.BICUBIC


class NcaltechDataset(Dataset):
    def __init__(self, cfg, mode):
        super(NcaltechDataset).__init__()
        self.cfg = cfg
        self.mode = mode

        self.Image_tran = ToPILImage()
        self.clip_transform = Compose([
                Resize(224, interpolation=BICUBIC),
                CenterCrop(224),
                _convert_image_to_rgb,
                ToTensor(),
                Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711)),
            ])

        if mode == 'val' or mode == 'test':
            val_path = Path(self.cfg.val_path)
            self.dataset_name = cfg.val_dataset
        else:
            raise AttributeError('Ncaltech can only use test or val mode.')

        # get class names
        self.class_map = sorted([cls.stem for cls in val_path.iterdir()])
        self.class_num = len(self.class_map)
        self.tonic_dtype = np.dtype([("x", int), ("y", int), ("t", int), ("p", int)])
        
        # get event file paths and labels
        self.file_path_list = []
        self.label_list = []
        for label, cls in enumerate(self.class_map):
            for file_path in (val_path / cls).iterdir():
                self.file_path_list.append(file_path)
                self.label_list.append(label)
        assert len(self.file_path_list) == len(self.label_list)

        # update class_name
        NEW_CNAMES = {
            "airplanes": "airplane",
            "Faces": "face",                    # actually doesn't exist
            "Faces_easy": "face",
            "Leopards": "leopard",
            "Motorbikes": "motorbike",
            "BACKGROUND_Google": "background",  # random images, hard to categorize
        }
        for idx, cls in enumerate(self.class_map):
            if cls in NEW_CNAMES:
                self.class_map[idx] = NEW_CNAMES[cls]

        # Gray or R-B
        self.colorization_type = getattr(cfg, 'colorization_type', None)
        if self.colorization_type == 'to_rgb':
            self.red = np.array([255, 0, 0])
            self.blue = np.array([0, 0, 255])
        elif self.colorization_type == 'to_gray':
            self.red = np.array([127, 127, 127])
            self.blue = np.array([127, 127, 127])

        np.seterr(divide='ignore',invalid='ignore')


    def __getitem__(self, idx):
        label = self.label_list[idx]
        event_path = self.file_path_list[idx]
        event = np.load(event_path).astype(np.float32)  # s, -1/1

        # Center shift event
        event = center_event(event, resolution=(180, 240))  

        # Split event
        event = split_event(event, 20000)

        # event to hist
        event = make_event_histogram(event, (180, 240), self.red, self.blue, background_mask=self.cfg.background_mask, count_non_zero=self.cfg.count_non_zero)

        event = self.Image_tran(event)
        
        # debug input
        if self.cfg.debug_input:
            event.save('./event.png')

        # Transform event
        event = self.clip_transform(event)

        return event, label

    def __len__(self):
        return len(self.file_path_list)







